  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="border-b border-gold/20 bg-card py-20 pt-32">
        <div className="container mx-auto px-4 text-center lg:px-8">
          ...
          <p className="mx-auto max-w-3xl text-xl text-muted-foreground">
            İstanbul'un dört bir yanında gerçekleştirdiğimiz profesyonel drone çekimlerinden 
            örnekleri keşfedin. Her proje, hikayesini havadan anlatıyor.
          </p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 lg:px-8">
          <div className="mb-8 flex flex-wrap justify-center gap-3">
            {categories.map((cat) => (
              <Button
                ...
                className={filter === cat.id ? "bg-gold text-background hover:bg-gold-dark" : ""}
              >
                {cat.name}
              </Button>
            ))}
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredItems.map((item) => (
              <div
                key={item.id}
                className="group relative overflow-hidden rounded-lg border border-gold/20 bg-card transition-transform hover:scale-105"
              >
                  ...
                  {item.type === "video" ? (
                    <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
                      <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gold/90 backdrop-blur-sm transition-transform group-hover:scale-110">
                        <Play className="ml-1 h-8 w-8 text-background" fill="currentColor" />
                      </div>
                    </div>
                  ) : (
      ...
      <section className="border-t border-gold/20 bg-card py-20">
        <div className="container mx-auto px-4 text-center lg:px-8">
          <h2 className="mb-6 text-3xl font-bold text-gold">Siz de Portföyümüze Katılın</h2>
          <p className="mx-auto mb-8 max-w-2xl text-lg text-muted-foreground">
            Projenizi havadan çekerek unutulmaz görüntüler elde edin. 
            Hemen rezervasyon yapın ve işinizi bir üst seviyeye taşıyın.
          </p>
